...
console.log(path.join(process.cwd(), 'TF', 'tfjs_bert', 'model.json'));
console.log(
  fs.existsSync(path.join(process.cwd(), 'TF', 'tfjs_bert', 'model.json'))
);
const model = await tf.loadLayersModel(
  path.join(process.cwd(), 'TF', 'tfjs_bert', 'model.json')
);
model.summary();
